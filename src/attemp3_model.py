...
# df = pickle.load(open(Path('..\artifacts\target_associated_data.pkl'), 'rb'))
...
pipeline = Pipeline(steps=[('stage1', Stage1Classifier(base_model=DecisionTreeClassifier(random_state=42))),
                           ('stage2', Stage2Classifier(p1_p3_model=DecisionTreeClassifier(random_state=42), 
...
import dill
dill.dump(pipeline, open(r'..\deliverables\model_v1.dill', 'wb'))

joblib.dump(pipeline, '..\deliverables\model_v1.joblib')

filename = '..\deliverables\model_v1.vas'
pickle.dump(pipeline, open(filename, 'wb'))
pickle.dump((X_test, y_test), open(r'..\artifacts\test_data.pkl', 'wb'))